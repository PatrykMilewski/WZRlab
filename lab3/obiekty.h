...
public:
	...
	Teren *teren;             // wskaŸnik do terenu, do którego przypisany jest obiekt
...
public:
	...
	ObiektRuchomy **wob;                          // tablica obiektów ruchomych, które mog¹ znaleŸæ siê w obszarze w ci¹gu
	...
	float wysokosc_max;             // maksymalna wysokoœæ terenu w sektorze -> istotna dla wyznaczenia rozdzielczoœci np. przy widoku z góry
	...
	void wstaw_obiekt_ruchomy(ObiektRuchomy *o);
	void usun_obiekt_ruchomy(ObiektRuchomy *o);
...
struct ParamFaldy
{
	...
	int sposob_laczenia_faudy;        // sposób ³¹czenia nowoutworzonej fa³dy z obecn¹ powierzchni¹ terenu 
};
...
public:
	...
	// inne istotne w³aœciwoœci terenu:
	...
	bool czy_toroidalnosc;        // inaczej cyklicznosc -> po dojsciu do granicy przeskakujemy na
	                              // pocz¹tek terenu, zarówno w pionie, jak i w poziomie (gdy nie ma granicy, teren nie mo¿e byæ cykliczny!)
	float granica_x, granica_z;   // granica terenu (jeœli -1 - teren nieskoñczenie wielki)
	...
	void UmieszczeniePrzedmiotuWterenie(Przedmiot *prz);     // m.i. ustalenie wysokoœci, na której le¿y przedmiot w zal. od ukszta³t. terenu
	...
	void WstawObiektWsektory(ObiektRuchomy *ob);
	void UsunObiektZsektorow(ObiektRuchomy *ob);
	long Obiekty_w_promieniu(ObiektRuchomy*** wsk_ob, Wektor3 pol, float promien);
};