...
{                                                // dzia³aj¹cych si³ i czasu, jaki up³yn¹³ od ostatniej symulacji
	...
	// wjazd lub zjazd: 
	//wPol.y = teren.WysokoscGruntu(wPol.x,wPol.z);   // najprostsze rozwi¹zanie - obiekt zmienia wysokoœæ bez zmiany orientacji
	...
	//Pt.y = teren->WysokoscGruntu(P.x, P.z); Qt.y = teren->WysokoscGruntu(Q.x, Q.z);
	//Rt.y = teren->WysokoscGruntu(R.x, R.z); St.y = teren->WysokoscGruntu(S.x, S.z);
	Pt.y = teren->Wysokosc(P); Qt.y = teren->Wysokosc(Q);
	Rt.y = teren->Wysokosc(R); St.y = teren->Wysokosc(S);
	...
	// korekta po³o¿enia w przypadku terenu cyklicznego (toroidalnego) z uwzglêdnieniem granic:
	if (teren->czy_toroidalnosc)
	{
		if (teren->granica_x > 0)
			if (wPol.x < -teren->granica_x) wPol.x += teren->granica_x*2;
			else if (wPol.x > teren->granica_x) wPol.x -= teren->granica_x*2;
		if (teren->granica_z > 0)
			if (wPol.z < -teren->granica_z) wPol.z += teren->granica_z*2;
			else if (wPol.z > teren->granica_z) wPol.z -= teren->granica_z*2;
	}
	else 
	{
		if (teren->granica_x > 0)
			if (wPol.x < -teren->granica_x) wPol.x = -teren->granica_x;
			else if (wPol.x > teren->granica_x) wPol.x = teren->granica_x;
		if (teren->granica_z > 0)
			if (wPol.z < -teren->granica_z) wPol.z = -teren->granica_z;
			else if (wPol.z > teren->granica_z) wPol.z = teren->granica_z;
	}
	...
	long liczba_prz_w_prom = teren->Przedmioty_w_promieniu(&wsk_prz, wPol, this->promien * 2);
	...
	long liczba_ob_w_promieniu = teren->Obiekty_w_promieniu(&wsk_ob, wPol, this->promien * 2.2 + wV.dlugosc()*dt);
	...
	for (long i = 0; i < liczba_prz_w_prom; i++)
	{
		...
		{			
			// bardzo duze uproszczenie -> traktuje pojazd jako kulê
			Wektor3 A = teren->p[prz->param_i[0]].wPol, B = teren->p[prz->param_i[1]].wPol;       // punkty tworz¹ce krawêdŸ
			A.y += teren->p[prz->param_i[0]].wartosc;
			B.y += teren->p[prz->param_i[1]].wartosc;
			...
			if (czy_kolizja_zreal == false)  // jeœli kolizja nie zosta³a jeszcze zrealizowana, sprawdzam kolizjê ze œcian¹ doln¹
			{
				...
				//	teren->liczba_wyswietlen, odl_zezn_prawa, x, QQ.x, QQ.y, QQ.z, N.x, N.y, N.z, Ad.x, Ad.y, Ad.z, wPol.x, wPol.y, wPol.z);
				//fprintf(f, "nr. cyklu = %d, odl_zezn_prawa = %f, odl do RR = %f, cos_kata_pad = %f\n",
				//	teren->liczba_wyswietlen, odl_zezn_prawa, xx, cos_kata_pad);
	...
	{
		Przedmiot *prz = &teren->p[i];
		if ((prz->do_wziecia == 0) && (prz->czy_ja_wzialem) && (prz->czy_odnawialny) &&
			(czas_symulacji - prz->czas_wziecia >= teren->czas_odnowy_przedm))
...
Sektor::Sektor(int _loczek, long _w, long _k, bool czy_mapa)
{
	...
	wob = new ObiektRuchomy*[liczba_obiektow_ruch_max];
...
void Sektor::pamiec_dla_mapy(int __liczba_oczek, bool czy_edycja)
{
	...
	int liczba_rozdzielczosci = 1+__log2(__liczba_oczek);         // liczba map o ró¿nych rozdzielczoœciach a¿ do mapy 1x1 (jedno oczko, 4 normalne)
...
void Sektor::zwolnij_pamiec_dla_mapy(bool czy_edycja)
{
	...
	if (mapa)
	{
		...
		long liczba_rozdzielczosci = 1 + __log2(loczek);
		for (int rozdz = 0; rozdz < liczba_rozdzielczosci; rozdz++)
		{
...
void Sektor::wstaw_obiekt_ruchomy(ObiektRuchomy *o)
{
	if (liczba_obiektow_ruch == liczba_obiektow_ruch_max)
	{
		ObiektRuchomy **wob_nowe = new ObiektRuchomy*[2 * liczba_obiektow_ruch_max];
...
void Sektor::usun_obiekt_ruchomy(ObiektRuchomy *o)
{
...
void Sektor::oblicz_normalne(float rozmiar_sektora, bool czy_edycja)
{
	...
	if (mapa)
	{
		long liczba_rozdzielczosci = 1 + __log2(loczek);              // gdy podst. rozdzielczoœæ 16x16, mamy 5 rozdzielczoœci:
...
{
	bool czy_z_pliku = 1;
    szczeg = 0.6;                 // stopieñ szczegó³owoœci wyœwietlania przedmiotów i powierzchni terenu (1 - pe³na, 0 - minimalna)
	liczba_wyswietlen = 0;
	ts = new TabSektorow();
	...
	if (czy_z_pliku)
	{
		int wynik = Odczyt("teren.map");
    if (wynik == -1) Odczyt("..\\teren.map");

		//Odczyt("plaski1.map");
	}
	else
	{
		rozmiar_sektora = 480;         // d³ugoœæ boku kwadratu w [m]
	...
	for (long i = 0; i < liczba_przedmiotow; i++)
		...
		{
			//p[i].wartosc = 20;
			//this->UmieszczeniePrzedmiotuWterenie(&p[i]);
		}
...
int Teren::Zapis(char nazwa_pliku[])
{
...
int Teren::Odczyt(char nazwa_pliku[])
{
	...
	fprintf(f, "\nOdczyt danych o terenie z pliku %s\n\n", nazwa_pliku);
	...
	for (long i = 0; i < _liczba_przedm; i++)
	{
		...
		UmieszczeniePrzedmiotuWterenie(&p[i]);
		if (p[i].czy_zazn) {
			p[i].czy_zazn = 0;
	...
	fprintf(f, "Koniec wczytywania danych o terenie.\n");
	fclose(pl);
	return 1;
}

void Teren::WspSektora(long *w, long *k, float x, float z)  // na podstawie wsp. po³o¿enia punktu (x,z) zwraca wsp. sektora
{
...
void Teren::PolozeniePoczSektora(float *x, float *z, long w, long k) // na podstawie wspó³rzêdnych sektora (w,k) 
...
float Teren::wysokosc_na_najw_zazn_przedm(Wektor3 pol)
{
...
// iteracyjne wyznaczenie kursora 3D z uwazglêdnieniem wysokoœci terenu i zaznaczonych przedmiotów, na których
// mo¿e le¿eæ nowo-tworzony przedmiot
Wektor3 Teren::wspolrzedne_kursora3D_bez_paralaksy(int X, int Y)
{
...
void Teren::UmieszczeniePrzedmiotuWterenie(Przedmiot *prz) // wyzn. par. przedmiotu w odniesieniu do terenu
{
...
long Teren::WstawPrzedmiot(Przedmiot prz)
{
	...
	UmieszczeniePrzedmiotuWterenie(&p[ind]);
	WstawPrzedmiotWsektory(&p[ind]);
	//fprintf(f,"wstawiam przedmiot %d w miejsce (%f, %f, %f)\n",i,teren.p[ind].wPol.x,teren.p[ind].wPol.y,teren.p[ind].wPol.z);
...
void Teren::ZaznaczOdznaczPrzedmiotLubGrupe(long nr_prz)
{
...
float Teren::WysokoscGruntu(float x, float z)      // okreœlanie wysokoœci dla punktu o wsp. (x,z) 
{
	...
	if (mapa)
	{
		...
		if ((k_lok < 0) || (w_lok < 0))
		{
			fprintf(f,"procedura Teren::Wysokosc: k i w lokalne nie moga byc ujemne!\n");
			fclose(f);
			exit(1);
		}
		...
		{
			fprintf(f, "procedura Teren::Wysokosc: k i w lokalne nie moga byc wieksze od liczby oczek - 1!\n");
			fclose(f);
			exit(1);
		}
...
float Teren::WysokoscNaPrzedmiocie(Wektor3 pol, Przedmiot *prz)
{
	float wys_na_p = -1e10;
	if (prz->typ == PRZ_MUR)
	{
...
float Teren::Wysokosc(Wektor3 pol)                 
{
	// szukamy przedmiotów w promieniu
	Przedmiot **wsk_prz = NULL;
	long liczba_prz_w_prom = Przedmioty_w_promieniu(&wsk_prz, pol, 0);
...
void Teren::WstawPrzedmiotWsektory(Przedmiot *prz)
{
...
long Teren::Przedmioty_w_promieniu(Przedmiot*** wsk_prz, Wektor3 pol, float promien)
{
	Wektor3 pol2D = pol;
	pol2D.y = 0;
...
void Teren::WstawObiektWsektory(ObiektRuchomy *ob)
{
...
void Teren::UsunObiektZsektorow(ObiektRuchomy *ob)
{
	float x1 = ob->wPol.x - ob->promien, x2 = ob->wPol.x + ob->promien,
		z1 = ob->wPol.z - ob->promien, z2 = ob->wPol.z + ob->promien;
...
void Teren::UsunPrzedmiotZsektorow(Przedmiot *prz)
{
	float x1 = prz->wPol.x - prz->srednica / 2, x2 = prz->wPol.x + prz->srednica / 2,
		z1 = prz->wPol.z - prz->srednica / 2, z2 = prz->wPol.z + prz->srednica / 2;
...
long Teren::Obiekty_w_promieniu(ObiektRuchomy*** wsk_ob, Wektor3 pol, float promien)
{
	...
	{
		(*wsk_ob) = new ObiektRuchomy*[liczba_obiektow_max];              // alokacja pamieci dla wskaŸników obiektów ruchomych
		for (long n = 0; n < liczba_obsz; n++)
		{
			Sektor *ob = oby[n];
...
float ilosc_pikseli_widocznosci(Wektor3 WPol, float srednica)
{
	...
	for (int pkt = 0; pkt<liczba_pkt_klucz; pkt++)
	{
		...
		float ilo_skal = (wPol2 - pol_kam) ^ pocz_kierunek_kamery;                      // iloczyn skalarny kierunku od kam do pojazdu i kierunku patrzenia kam
...
void Teren::Rysuj()
{
	...
	UstawieniaKamery(&pol_kam, &kierunek_kam, &pion_kam);             // wyznaczenie po³o¿enia kamery i innych jej ustawieñ
					...
					case PRZ_DRZEWO:
					{
						...
						case DRZ_TOPOLA:
						{
							...
							else
							{
								...
								gluCylinder(Qcyl, sredn / 2, sredn, wysokosc*1.1, liczba, liczba * 2);
								glPopMatrix();
						...
						case DRZ_SWIERK:
						{
							...
							else
							{
								...
								gluCylinder(Qcyl, promien, promien * 2, wysokosc*1.1, liczba, liczba * 2);
