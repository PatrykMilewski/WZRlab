...
ObiektRuchomy::ObiektRuchomy(Teren *teren)
{
	...
	dlugoscOstatniejPrzerwy = 0;
	czasOstatniejRamki = clock();
	F = Fb = alfa = 0;	// si³y dzia³aj¹ce na obiekt 
	F_max = 10000;
	alfa = 0;
	alfa_max = PI*30.0 / 180;
	ham = 0;			// stopieñ hamowania
	masa_wlasna = 800.0;			// masa obiektu [kg]
	ilosc_paliwa = 5.0;
	...
	fprintf(f, "Polozenie poczatkowe mojego obiektu = (%f, %f, %f)\n", wPol.x, wPol.y, wPol.z);
...
{                                                // dzia³aj¹cych si³ i czasu, jaki up³yn¹³ od ostatniej symulacji
	...
	float tarcie_toczne = 0.10;    // wspó³czynnik tarcia tocznego
	float sprezystosc = 0.2;       // wspó³czynnik sprê¿ystoœci (0-brak sprê¿ystoœci, 1-doskona³a sprê¿ystoœæ) 
...
Teren::Teren()
{
  ...
  for (long i = 0; i < siatka.liczba_wezlow; i++) 
    siatka.wezly[i].normalna = Wektor3(0,0,0);
    ...
    siatka.wezly[siatka.sciany[i].ind1].normalna = siatka.wezly[siatka.sciany[i].ind1].normalna + siatka.sciany[i].normalna*0.3333;
    siatka.wezly[siatka.sciany[i].ind2].normalna = siatka.wezly[siatka.sciany[i].ind2].normalna + siatka.sciany[i].normalna*0.3333;
    siatka.wezly[siatka.sciany[i].ind3].normalna = siatka.wezly[siatka.sciany[i].ind3].normalna + siatka.sciany[i].normalna*0.3333;
