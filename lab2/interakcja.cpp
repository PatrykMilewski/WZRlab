...
FILE *f = fopen("WZR_nawigacja.txt","w");    // plik do zapisu informacji testowych
...
long moment_wyslania = -1;           // moment wys³ania ostatnij ramki
...
float scenariusz_testu[][3] = { { 10, 10000, 0 }, { 5, 20000, PI * 20 / 180 }, { 10, -7000, -PI * 20 / 180 }, { 5, 0, 0 } }; // {czas, si³a, k¹t skrêtu kó³} -> przez jaki czas obiekt ma sie poruszaæ z podan¹ prêdkoœci¹ i k¹tem skrêtu kó³
long nr_akcji = 0;
  ...
  while(true)
  {
    ...
    case STAN_OBIEKTU:
      ...
      //if (stan.iID != pMojObiekt->iID)      // jeœli to nie mój w³asny obiekt - zakomentowane by uzyskaæ widmo sieciowe
      {
		...
		CudzeObiekty[IndeksyOb[stan.iID]]->dlugoscOstatniejPrzerwy = ramka.moment_wyslania - CudzeObiekty[IndeksyOb[stan.iID]]->czasOstatniejRamki;
		CudzeObiekty[IndeksyOb[stan.iID]]->czasOstatniejRamki = ramka.moment_wyslania;
...
void PoczatekInterakcji()
{
  ...
  multi_reciv = new multicast_net("224.12.12.97",10001);      // obiekt do odbioru ramek sieciowych
  multi_send = new multicast_net("224.12.12.97",10001);       // obiekt do wysy³ania ramek

  if (czy_opoznienia)  
  {
    float srednie_opoznienie = 1.7*(float)rand()/RAND_MAX, wariancja_opoznienia = 0.4; // parametry opóŸnieñ
...
void Cykl_WS()
{
  ...
  {                              // nale¿y na nowo obliczyæ œredni czas cyklu fDt
    ...
    if (licznik_sym > 500)
    {
      sprintf(text,"2017/18 (h) %0.0f fps  %0.2fms  œr.czêstoœæ = %0.2f[r/s]  œr.odl = %0.3f[m]  œr.odl.k¹t. = %0.3f[st]",
        fFps,1000.0/fFps,sr_czestosc,sum_roznic_pol/licznik_sym,sum_roznic_kat/licznik_sym*180.0/3.14159);
	    if ((float)(clock() - czas_start) / CLOCKS_PER_SEC > 10)
  ...
  long moment_biezacy = clock();
  if ((moment_biezacy - moment_wyslania >= CLOCKS_PER_SEC) || !czy_zmn_czestosc)
  {    
    moment_wyslania = moment_biezacy;
    Ramka ramka;                                    
    ramka.stan = pMojObiekt->Stan();               // stan w³asnego obiektu 
    ramka.moment_wyslania = moment_wyslania;
  ...
  {
    // TODO: Tutaj powinien znaleŸæ siê kod predykcji

	  ObiektRuchomy* source = CudzeObiekty[i];
	  long tx = clock();
	  long t0 = source->czasOstatniejRamki;
	  if (tx == t0) continue;

	  long t1 = t0 + source->dlugoscOstatniejPrzerwy; // mozna tez np. jakas srednia z kilku ostatnich ramek
	  Wektor3 v0 = source->ostatniStan.wV;
	  Wektor3 v1 = v0 + source->wA * fDt;
	  source->wV = v0 + (v1 - v0) / (t1 - t0)*(tx - t0);

	  Wektor3 dwPol = source->wV * fDt + source->wA * fDt*fDt / 2;
	  source->wPol = source->wPol + dwPol;

	  Wektor3 o0 = source->ostatniStan.wV_kat;
	  Wektor3 o1 = o0 + source->wA_kat * fDt;
	  source->wV_kat = o0 + (o1 - o0) / (t1 - t0)*(tx - t0);

	  Wektor3 w_obrot = source->wV_kat * fDt + source->wA_kat * fDt * fDt / 2;
	  kwaternion q_obrot = AsixToQuat(w_obrot.znorm(), w_obrot.dlugosc());
	  source->qOrient = q_obrot * source->qOrient;
  }
}
  ...
  case WM_RBUTTONDOWN: //reakcja na prawy przycisk myszki
    {
      ...
      int y = HIWORD(lParam);
      if (sterowanie_myszkowe)
        pMojObiekt->F = -pMojObiekt->F_max;        // si³a pchaj¹ca do tylu
  ...
  case WM_KEYDOWN:
    {
      ...
      case VK_DOWN:
        {
          pMojObiekt->F = -pMojObiekt->F_max;
          break;
        }
      case VK_LEFT:
        {
          if (SHIFTwcisniety) pMojObiekt->alfa = pMojObiekt->alfa_max;
          else pMojObiekt->alfa = pMojObiekt->alfa_max/4;
