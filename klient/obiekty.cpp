...
ObiektRuchomy::ObiektRuchomy(Teren *teren)
{
  ...
  //iID = (unsigned int)(clock() % 1000);  // identyfikator obiektu
	...
	masa_wlasna = 5.0;			// masa obiektu [kg]
  ilosc_paliwa = 3.0;
...
{                                                // dzia³aj¹cych si³ i czasu, jaki up³yn¹³ od ostatniej symulacji
  ...
  float sprezystosc = 0.9;       // wspó³czynnik sprê¿ystoœci (0-brak sprê¿ystoœci, 1-doskona³a sprê¿ystoœæ) 
  ...
  Wektor3 S_PQR = Rt + (Pt-Rt)/2 + (Qt-Pt)/2 + normPQR*wysokosc/2,     // po³o¿enie œrodka pokazdu gdy punkty P,Q,R stykaj¹ siê z pod³o¿em
  ...
  if (((P - teren->srodek).dlugosc() <= (Pt - teren->srodek).dlugosc()+ wysokosc/2+przeswit)||
  ...
  if (((P - teren->srodek).dlugosc() <= (Pt - teren->srodek).dlugosc()+ wysokosc/2+przeswit)+
      ((Q - teren->srodek).dlugosc() <= (Qt - teren->srodek).dlugosc()+ wysokosc/2+przeswit)+
      ((R - teren->srodek).dlugosc() <= (Rt - teren->srodek).dlugosc()+ wysokosc/2+przeswit)+
      ((S - teren->srodek).dlugosc() <= (St - teren->srodek).dlugosc()+ wysokosc/2+przeswit) > 2)
...
Teren::Teren()
{
   //int wynik = tworz_z_obj(&siatka,"sfera3D_8.obj");      // wczytanie siatki tójk¹tów z formatu *.obj (Wavefront)
   int wynik = tworz_z_obj(&siatka,"planeta3.obj");      // wczytanie siatki tójk¹tów z formatu *.obj (Wavefront)
   if (wynik == 0)
     int wynik = tworz_z_obj(&siatka,"..//planeta3.obj");      // wczytanie siatki tójk¹tów z formatu *.obj (Wavefront)

   //int wynik = tworz_z_obj(&siatka,"sfera6.obj");      // wczytanie siatki tójk¹tów z formatu *.obj (Wavefront)
   float skala = 5000.0;         
